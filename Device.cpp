...
void Device::initialize(double max_frequency)
{
...
double Device::determine_grid_spacing(double max_frequency)
{
	double delta_lambda = determine_wavelength_resolution(m_layer_permittivities, m_layer_permeabilities, max_frequency);
	auto critical_dimension_iter = std::min_element(std::begin(m_layer_widths), std::end(m_layer_widths));

	double critical_dimension{};
	if (critical_dimension_iter != std::end(m_layer_widths))
	{
		critical_dimension = *critical_dimension_iter;
	}

	double delta_d = determine_device_resolution(critical_dimension);
	double grid_step_size = snap_grid(critical_dimension, delta_lambda, delta_d);

	return grid_step_size;
}

double Device::determine_wavelength_resolution(const std::vector<double>& layer_permittivities, const std::vector<double>& layer_permeabilities, double max_frequency)
{
	assert(layer_permittivities.size() == layer_permeabilities.size());

	uint32_t wavelength_resolution = 20;

	std::vector<double> layer_indices_of_refraction = sqrt(layer_permittivities * layer_permeabilities);

	double max_index_of_refraction = max(layer_indices_of_refraction, 1.0);

	double lambda_min = c / (max_frequency * max_index_of_refraction);
	double delta_lambda = lambda_min / wavelength_resolution;

	return delta_lambda;
}

double Device::determine_device_resolution(double critical_dimension)
{
	double device_resolution = 4; // Number of points to resolve device geometry with
	double delta_d = critical_dimension / device_resolution;

	return delta_d;
}

double Device::snap_grid(double critical_dimension, double delta_lambda, double delta_d)
{
	double grid_step_size_unsnapped = std::min(delta_lambda, delta_d);
	double grid_size_unsnapped = critical_dimension / grid_step_size_unsnapped;
	uint32_t device_size = round_cells_up(grid_size_unsnapped);
	double grid_step_size = critical_dimension / device_size;

	return grid_step_size;
}

uint32_t Device::round_cells_up(double unrounded_number_of_cells)
{
...
void Device::generate_material_grid(std::vector<double>& epsilon_r, std::vector<double>& mu_r)
{
...
void Device::generate_grid_1D(std::vector<double>& grid)
{
	double zmin = 0.0;
	double zmax = m_full_grid_size * m_grid_resolution;
